...
go-tunnel 提供了完整的客户端-服务器隧道实现，支持类似 ngrok/frp 的内网穿透功能。

### v1.1.1 推荐方案：QUIC 原生多路复用

从 v1.1.1 开始，推荐使用 `quic` 包的 `MuxServer` 和 `MuxClient`，它们提供了：

- **原生 QUIC 多路复用**：无 HTTP/3 开销，封包效率提升 3-10 倍
- **高可用组件集成**：熔断器、重试机制、资源限制器
- **自动重连**：指数退避重试，支持抖动防止惊群效应
- **安全认证**：常量时间比较防止时序攻击
...
```
外部用户 ──▶ 公网端口 ──▶ MuxServer ──▶ 控制流 ──▶ MuxClient ──▶ 本地服务
                              │
                              └── QUIC Stream (原生多路复用)
```
...
func main() {
    ...
    log.Printf("QUIC tunnel server started on :443")
...
func main() {
    ...
    log.Printf("Tunnel established: %s", client.PublicURL())
...
`quic.MuxServer` 内置了以下高可用组件：

| 组件 | 说明 | 配置 |
|------|------|------|
| 熔断器 | 防止级联故障 | 失败阈值 5，超时 30s |
| 连接限制器 | 防止资源耗尽 | `MaxTunnels * MaxConnsPerTunnel` |
| 缓冲池 | 减少 GC 压力 | 64KB 默认缓冲区 |
| 健康检查 | 超时隧道清理 | 30s 检查间隔 |

`quic.MuxClient` 内置了以下高可用组件：

| 组件 | 说明 | 配置 |
|------|------|------|
| 重试机制 | 指数退避重连 | 初始延迟 5s，最大 5min |
| 背压控制 | 防止内存溢出 | 自动水位管理 |
| 缓冲池 | 减少 GC 压力 | 64KB 默认缓冲区 |
| 心跳保活 | 保持连接活跃 | 15s 心跳间隔 |
...
| 指标 | HTTP/3 | 纯 QUIC | 提升 |
|------|--------|---------|------|
| 封包开销 | 50-100 bytes | 7-15 bytes | 3-10x |
| 延迟 | 50-100ms | 20-50ms | ~50% |
| 吞吐量 | ~6 Gbps | ~8 Gbps | ~30% |
...
server := quic.NewMuxServer(quic.MuxServerConfig{
    ...
    AuthToken:        "secret",         // 认证令牌
    PortRangeStart:   10000,            // TCP 端口范围起始
    PortRangeEnd:     20000,            // TCP 端口范围结束
    MaxTunnels:       10000,            // 最大隧道数
    MaxConnsPerTunnel: 1000,            // 每隧道最大连接数
    TunnelTimeout:    5 * time.Minute,  // 隧道超时
})
...
// stats.ActiveTunnels, stats.ActiveConns, stats.BytesIn, stats.BytesOut
...
// 创建客户端
client := quic.NewMuxClient(quic.MuxClientConfig{
    ServerAddr:        "tunnel.example.com:443", // 服务器地址
    TLSConfig:         tlsConfig,                // TLS 配置
    TunnelID:          "my-tunnel",              // 隧道 ID（可选）
    Protocol:          quic.ProtocolTCP,         // 协议类型
    LocalAddr:         "localhost:8080",         // 本地服务地址
    AuthToken:         "secret",                 // 认证令牌
    ReconnectInterval: 5 * time.Second,          // 重连间隔
    MaxReconnectTries: 0,                        // 最大重试次数（0=无限）
})
...
以下示例展示如何使用 `forward/mux.go` 构建自定义隧道：
